  ...
  public boolean isValid(Order domain, ConstraintValidatorContext context) {
    ...
    if (domain.getOrderEvents().size() <= 0) {
      ...
      context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("{order.orderEvents.min1}")
    ...
    if (domain.getOrderDetails().size() <= 0) {
      ...
      context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("{order.orderDetails.min1}")
            .addConstraintViolation();
    }
    ...
    if (domain.getIsGuest() && domain.getUser() != null) {
      ...
      context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("{order.user.null}")
            .addConstraintViolation();
    }
    ...
    if (!domain.getIsGuest() && domain.getUser() == null) {
      ...
      context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("{order.user.notnull}")
            .addConstraintViolation();
    }
