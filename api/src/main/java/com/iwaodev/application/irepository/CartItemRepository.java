...
public interface CartItemRepository extends JpaRepository<CartItem, Long>, JpaSpecificationExecutor<CartItem> {
  ...
  Page<CartItem> getAllByUserIdWithPage(UUID userId, Pageable pageable);
   ...
   **/
  //@Query(value = "SELECT EXISTS (SELECT 1 FROM cart_items c WHERE c.variant_id = ?1 AND c.user_id = ?2)", nativeQuery = true)
