...
usage() { echo "Usage: $0 [-i <image name:string>] [-v <version number:string>] [-t <docker target stage name:string>]" 1>&2; exit 1; }


while getopts ":i:v:t:" opt; do
  case ${opt} in
    v ) # process option h
      v=${OPTARG}
      ;;
...
if [ -z "${t}" ]; then
  docker build --tag=${i}:${v} .
else
  docker build --tag=${i}:${v} --target=${t} .
fi
