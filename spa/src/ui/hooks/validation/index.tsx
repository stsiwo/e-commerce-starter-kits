...
export const useValidation = <D extends Record<string, unknown>>(
  input: UseValidationInputType<D>
): UseValidationOutputType<D> => {
  const updateValidationAt: (path: string, value: any) => void = (
    path,
    value
  ) => {
    const tempValidationData: DomainValidationType<D> = cloneDeep(
      input.curValidationDomain
    );
    ...
    input.schema
      .validateAt(path, tempDomainData)
      .then(() => {
        log("passed finally");
   ...
   *
   **/
  const updateValidationAtMultiple: (
    list: ValidationKeyValuePiarType[]
  ) => void = (list) => {
    const tempValidationData: DomainValidationType<D> = cloneDeep(
      input.curValidationDomain
    );
    ...
    list.forEach((item: ValidationKeyValuePiarType) =>
      set(tempDomainData, item.key, item.value)
    );
  ...
  const updateAllValidation: () => void = async () => {
    ...
    log(tempEmptyValidationData);

    log("error object");
    await input.schema
      .validate(input.curDomain, { abortEarly: false })
      .then(() => {
        // success
      })
      .catch((error) => {
    ...
    input.setValidationDomain(
      tempEmptyValidationData as DomainValidationType<D>
    );
  };
