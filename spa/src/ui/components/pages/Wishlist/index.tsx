...
const Wishlist: React.FunctionComponent<{}> = (props) => {
  ...
  const curQueryString = useSelector(
    mSelector.makeWishlistItemQueryStringSelector()
  );
  const pagination = useSelector(
    mSelector.makeWishlistItemPaginationSelector()
  );
  ...
  const handleMoveToCartClick: React.EventHandler<
  ...
  > = (e) => {
    ...
    // validate the wishlist item has enough stock to be moved to the cart
    if (isOutOfStock(targetWishlistItem)) {
      dispatch(
        messageActions.update({
          id: getNanoId(),
          type: MessageTypeEnum.ERROR,
          message: "the wishlist item does not have any stock.",
        })
      );
      return false;
    }
  ...
  const renderWishlistItems: () => React.ReactNode = () => {
    return curWishlistItems.map((wishlistItem: WishlistItemType) => {
      return (
        <WishlistItem
          value={wishlistItem}
          onMoveToCartClick={handleMoveToCartClick}
  ...
  // spinner
  const curFetchWishlistFetchStatus = useSelector(
    rsSelector.app.getFetchWishlistItemFetchStatus
  );
  ...
  return (
    <React.Fragment>
      ...
      {curFetchWishlistFetchStatus === FetchStatusEnum.SUCCESS &&
