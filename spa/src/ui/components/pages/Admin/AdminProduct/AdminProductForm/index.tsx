...
import {
  ...
  generateDefaultProductOnlyData,
  ProductDataType,
  ProductType,
  ProductValidationDataType,
 ...
 **/
const AdminProductForm = React.forwardRef<any, AdminProductFormPropsType>(
  (props, ref) => {
    // mui: makeStyles
     ...
     *
     **/
    const [curProductState, setProductState] = React.useState<ProductDataType>(
      props.product
        ? merge({}, generateDefaultProductOnlyData(), props.product)
        : generateDefaultProductOnlyData()
    );
    ...
    const [isNew, setNew] = React.useState<boolean>(
      props.product ? false : true
    );

    // validation logic (should move to hooks)
    const [curProductValidationState, setProductValidationState] =
      React.useState<ProductValidationDataType>(defaultProductValidationData);
    ...
    // event handlers
    const handleProductNameInputChangeEvent: React.EventHandler<
    ...
    const handleProductDescriptionInputChangeEvent: React.EventHandler<
      React.ChangeEvent<HTMLInputElement>
    > = (e) => {
    ...
    const handleProductPathInputChangeEvent: React.EventHandler<
      React.ChangeEvent<HTMLInputElement>
    > = (e) => {
    ...
    const handleProductCategoryInputChangeEvent: React.EventHandler<
      React.ChangeEvent<HTMLInputElement>
    > = (e) => {
      ...
      const nextCategory = categoryList.find(
        (category: CategoryType) => nextCategoryId === category.categoryId
      );
      // must not be null
    ...
    const handleProductBaseUnitPriceInputChangeEvent: React.EventHandler<
      React.ChangeEvent<HTMLInputElement>
    > = (e) => {
    ...
    const handleProductNoteInputChangeEvent: React.EventHandler<
      React.ChangeEvent<HTMLInputElement>
    > = (e) => {
    ...
    const handleProductPublicChangeEvent: React.EventHandler<
      React.ChangeEvent<HTMLInputElement>
    > = (e) => {
    ...
    const updateProductImage = (file: File, path: string, index: number) => {
      ...
      const renamedFile = renameFile(
        file,
        curProductState.productImages[index].productImageName
      );
    ...
    return (
      <form className={classes.form} noValidate autoComplete="off">
        ...
        <TextField
          ...
          select
          value={
            curProductState.category ? curProductState.category.categoryId : "1"
          }
        ...
        <Typography
          ...
          className={classes.subtitle}
        >
          the base inforamtion takes over any variant informatiion if you don't
          specify these information on each variant.
        </Typography>
        <TextField
        ...
        <Typography
          ...
          className={classes.subtitle}
        >
          To publish this product, you need to add at least one variant. You can
          add as many variants as you want at its management page. Click the
          link at 'Variants' column of your target product.
        </Typography>
        <Box className={classes.errorMsg}>
