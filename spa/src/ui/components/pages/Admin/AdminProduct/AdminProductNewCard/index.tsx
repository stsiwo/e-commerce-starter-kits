 ...
 *
 **/
const AdminProductNewCard: React.FunctionComponent<AdminProductNewCardPropsType> =
  (props) => {
    // mui: makeStyles
    ...
    const handleNewProductFormToggleBtnClickEvent: React.EventHandler<
    ...
    const handleProductSaveBtnClickEvent: React.EventHandler<
      React.MouseEvent<HTMLButtonElement>
    > = async (e) => {
      log("passed");
    };
      ...
      <Card className={classes.root}>
        <CardHeader
          ...
          action={
            <IconButton
              aria-label="add"
              onClick={handleNewProductFormToggleBtnClickEvent}
            >
              {curFormOpen && <RemoveCircleIcon />}
              {!curFormOpen && <AddCircleIcon />}
            </IconButton>
          }
        />
