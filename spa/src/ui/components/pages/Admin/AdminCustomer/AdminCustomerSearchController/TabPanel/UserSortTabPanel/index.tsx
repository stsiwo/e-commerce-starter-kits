...
const UserSortTabPanel: React.FunctionComponent<{}> = (props) => {
  ...
  const handleUserSortInputChangeEvent: React.EventHandler<
    React.ChangeEvent<HTMLInputElement>
  > = (e) => {
    dispatch(
      userQuerySortActions.update(e.currentTarget.value as UserSortEnum)
    );
  };

  const renderUserSortRadioInputs: () => React.ReactNode = () => {
    log("renderUserSortRadioInputs updated");
    ...
    <Box p={3}>
      <FormControl component="fieldset">
        ...
        <RadioGroup
          ...
          name="user-sort"
          value={curSort}
          onChange={handleUserSortInputChangeEvent}
        >
