...
const CartBox: React.FunctionComponent<CartBoxPropsType> = (props) => {
   ...
   *
   */
  const handleGoSearchClick = (e: React.MouseEvent<HTMLElement>) => {
    dispatch(cartModalActions.close());
  };
        ...
        <React.Fragment>
          ...
          <Box component="div" className={classes.subtotalBox}>
            <Typography variant="subtitle1" component="h3" align="right">
              Subtotal (<b>{calcSubTotalProductNumbers(curCartItems)}</b>{" "}
              items):{" "}
              <b>{cadCurrencyFormat(calcSubTotalPriceAmount(curCartItems))}</b>
            </Typography>
          </Box>
          <Box component="div" className={classes.controllerBox}>
