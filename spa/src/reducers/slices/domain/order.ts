...
import {
  ...
  OrderSortEnum,
  OrderType,
  SessionTimeoutOrderEventCriteria,
...
export const fetchSingleOrderActionCreator = createAction<{ orderId: string }>(
  "saga/domain/order/fetchSingle"
);
export const fetchSingleOrderActionTypeName =
  fetchSingleOrderActionCreator().type;

// for POST (add order with clientSecret) request
export declare type PostOrderActionType = OrderCriteria;
export const postOrderActionCreator = createAction<PostOrderActionType>(
  "saga/domain/order/post"
);
export const postOrderActionTypeName = postOrderActionCreator().type;

// for PUT (replace) request
export const putOrderActionCreator = createAction<OrderType>(
  "saga/domain/order/put"
);
export const putOrderActionTypeName = putOrderActionCreator().type;

// for DELETE (delete single cart item) request
export const deleteSingleOrderActionCreator = createAction<OrderType>(
  "saga/domain/order/deleteSingle"
);
export const deleteSingleOrderActionTypeName =
  deleteSingleOrderActionCreator().type;

// for DELETE (delete all of cart items) request
export const deleteOrderActionCreator = createAction<OrderType>(
  "saga/domain/order/delete"
);
...
export const deleteSingleOrderEventActionCreator =
  createAction<DeleteSingleOrderEventActionType>(
    "saga/domain/order/event/deleteSingle"
  );
export const deleteSingleOrderEventActionTypeName =
  deleteSingleOrderEventActionCreator().type;

// for POST (add a new session timeout order event) request
export declare type PostSessionTimeoutOrderEventActionType =
  SessionTimeoutOrderEventCriteria & { orderId: string };
export const postSessionTimeoutOrderEventActionCreator =
  createAction<PostSessionTimeoutOrderEventActionType>(
    "saga/domain/order/event/session-timeout/post"
  );
export const postSessionTimeoutOrderEventActionTypeName =
  postSessionTimeoutOrderEventActionCreator().type;
/**
 *
...
export const orderSlice = createSlice({
  ...
  reducers: {
    ...
    delete: (state: OrderType[], action: PayloadAction<OrderType>) => {
      remove(
        state,
        (order: OrderType) => order.orderId == action.payload.orderId
      );
      return state;
    },
    ...
    appendEvent: (
      state: OrderType[],
      action: PayloadAction<{ orderId: string; event: OrderEventType }>
    ) => {
    ...
    replace: (
      state: OrderType[],
      action: PayloadAction<{ orderId: string; order: OrderType }>
    ) => {
      for (let i = 0; i < state.length; i++) {
        if (state[i].orderId == action.payload.orderId) {
    ...
    deleteEvent: (
      state: OrderType[],
      action: PayloadAction<{ orderId: string; eventId: string }>
    ) => {
      for (let i = 0; i < state.length; i++) {
        if (state[i].orderId == action.payload.orderId) {
          state[i].orderEvents = state[i].orderEvents.filter(
            (orderEvent: OrderEventType) =>
              orderEvent.orderEventId != action.payload.eventId
          );
        }
      }
      return state;
    ...
    updateEvent: (
      ...
      for (let i = 0; i < state.length; i++) {
        if (state[i].orderId == action.payload.orderId) {
          state[i].orderEvents = state[i].orderEvents.map(
            (orderEvent: OrderEventType) => {
              if (
                orderEvent.orderEventId == action.payload.event.orderEventId
              ) {
...
export const orderQuerySearchQuerySlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: string, action: OrderQuerySearchQueryActionType) =>
      action.payload,
    clear: (state: string) => "",
  },
  /**
...
export const orderQuerySearchQuerySliceReducer =
  orderQuerySearchQuerySlice.reducer;
...
export const orderQueryOrderStatusSlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: string, action: OrderQueryOrderStatusActionType) =>
      action.payload,
    clear: (state: string) => "",
  },
  /**
...
export const orderQueryOrderStatusSliceReducer =
  orderQueryOrderStatusSlice.reducer;
...
export const orderQueryStartDateSlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: string, action: OrderQueryStartDateActionType) =>
      action.payload,
...
export const orderQueryEndDateSlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: string, action: OrderQueryEndDateActionType) =>
      action.payload,
    clear: (state: string) => null,
  },
  /**
...
export const orderQuerySortSlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: OrderSortEnum, action: OrderQuerySortActionType) =>
      action.payload,
...
export const orderPaginationPageSlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: number, action: OrderPaginationPageActionType) =>
      action.payload,
...
export const orderPaginationLimitSlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: number, action: OrderPaginationLimitActionType) =>
      action.payload,
...
export const orderPaginationLimitSliceReducer =
  orderPaginationLimitSlice.reducer;
...
export const orderPaginationTotalPagesSlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: number, action: OrderPaginationTotalPagesActionType) =>
      action.payload,
...
export const orderPaginationTotalPagesSliceReducer =
  orderPaginationTotalPagesSlice.reducer;
export const orderPaginationTotalPagesActions =
  orderPaginationTotalPagesSlice.actions;
...
export const orderPaginationTotalElementsSlice = createSlice({
  ...
  reducers: {
    ...
    // use when you want to replace
    update: (state: number, action: OrderPaginationTotalElementsActionType) =>
      action.payload,
...
export const orderPaginationTotalElementsSliceReducer =
  orderPaginationTotalElementsSlice.reducer;
export const orderPaginationTotalElementsActions =
  orderPaginationTotalElementsSlice.actions;
