...
import { postWishlistItemFetchStatusActions } from "reducers/slices/app";
 ...
 *  
 **/
export function* postWishlistItemWorker(action: PayloadAction<WishlistItemType>) {
  ...
  if (curAuth.userType === UserTypeEnum.MEMBER) {
     ...
     * grab all domain
     **/
    const apiUrl = `${appConfig.baseUrl}/users/${curAuth.user.userId}/wishlistItems`
    ...
    try {
       ...
       **/
      yield put(
        wishlistItemActions.merge(response.data.data)
      )
     ...
     **/
    yield put(
      wishlistItemActions.merge([newEntity])
    )
