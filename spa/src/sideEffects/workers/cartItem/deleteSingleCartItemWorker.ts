...
import { deleteSingleCartItemFetchStatusActions } from "reducers/slices/app";
...
export function* deleteSingleCartItemWorker(action: PayloadAction<CartItemType>) {
  ...
  if (curAuth.userType === UserTypeEnum.MEMBER) {
     ...
     * grab all domain
     **/
    const apiUrl = `${appConfig.baseUrl}/users/${curAuth.user.userId}/cartItems/${action.payload.cartId}`
