...
-- Sat 01 May 2021 12:42:30 AM PDT
...
CREATE TABLE IF NOT EXISTS `ec-schema`.`users` (
  ...
  `company_name` VARCHAR(45) NULL COMMENT 'should refactor this.',
  PRIMARY KEY (`user_id`),
...
CREATE TABLE IF NOT EXISTS `ec-schema`.`addresses` (
  ...
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NULL,
  `is_sender_address` TINYINT(1) NULL COMMENT 'use this for admin user as sender address when shipping',
...
CREATE TABLE IF NOT EXISTS `ec-schema`.`orders` (
  ...
  INDEX `fk_orders_order_addresses1_idx` (`shipping_address` ASC),
  INDEX `fk_orders_order_addresses2_idx` (`billing_address` ASC),
